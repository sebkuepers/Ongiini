...
This module subclasses mem0's VllmLLM to:
  ...
  2. Post it to `app.usage.record()` against the msisdn in
     `_current_msisdn`
...
def install() -> None:
    ...
    Called from app.mem at module load, before any mem0 Memory is built.
    """
    global _installed
    if _installed:
        return
    ...
    LlmFactory.provider_to_class["vllm"] = ("app.mem_llm.TrackedVllmLLM", entry[1])
