...
from webhook.app.tools_pkg import (
...
async def test_fetch_urls_fans_out_in_parallel():
    with patch(
        "webhook.app.tools_pkg.ongiini_tools._fetch_url_impl",
...
async def test_whats_in_my_memory_returns_empty_message_for_clean_user():
    ...
    with patch("webhook.app.tools_pkg.ongiini_tools._memory.load", return_value=[]):
...
async def test_lookup_docs_falls_back_when_file_missing():
    ...
    from webhook.app.tools_pkg import ongiini_tools as ot
...
async def test_web_search_delegates_to_impl():
    with patch(
        "webhook.app.tools_pkg.ongiini_tools._web_search_impl",
...
async def test_my_token_usage_summarises_stats():
    ...
    with patch("webhook.app.tools_pkg.ongiini_tools._usage.summary_for", return_value=fake_stats):
